...
if __name__ == '__main__':
    ...
    parser.add_argument('start_idx', help='Start index', type=int) # 1
    parser.add_argument('end_idx', help='End index from the end', type=int) # -1 last index
    ...
    for name in names:
        ...
        print(complete_path)
        # print(complete_path)
        if "S1b" in complete_path or "S1c" in complete_path or "S2a" in complete_path or "S2b" in complete_path or "S2c" in complete_path:
            print("continue")
            continue
        ...
        first_996 = list(range(12, 1013))
        second_966= list(range(1036, 2037))
        total = first_996+ second_966

        # AMPLITUDE
        # can add list of column you want to choose. 
        csi_matrix_processed[:, total, 0] = np.abs(H_est[total, :]).T
        

        # PHASE
        phase_before = np.unwrap(np.angle(H_est[total, :]), axis=0)
        ...
        csi_matrix_processed[:, total, 1] = phase_before.T

        mdic = {"csi_matrix_processed": csi_matrix_processed[:, total, :]}
        print("saving")
